...
//
// Declaring these in AndroidManifest.xml is not enough. From API 31 the
// Bluetooth permissions are RUNTIME permissions, so an app that never asks gets
// a SecurityException from the first scan and — worse — looks like it simply
// cannot see any peers. ble-plx does not request them for you.

export const ANDROID_BLE_PERMS_API31 = [
  'android.permission.BLUETOOTH_SCAN',
  'android.permission.BLUETOOTH_CONNECT',
  'android.permission.BLUETOOTH_ADVERTISE',
];

/** Below API 31 BLE scanning is gated on fine location instead. */
export const ANDROID_BLE_PERMS_LEGACY = ['android.permission.ACCESS_FINE_LOCATION'];

/** Which permissions this Android level actually needs. Pure, unit-tested. */
export function androidPermissionsFor(sdkInt: number): string[] {
  return sdkInt >= 31 ? ANDROID_BLE_PERMS_API31 : ANDROID_BLE_PERMS_LEGACY;
}

export interface PermissionResult {
  granted: string[];
  denied: string[];
  /** true when the platform has no runtime permission model here (iOS/web/node). */
  unavailable: boolean;
}

/** Ask for exactly the permissions this platform needs. Never throws. */
export async function ensureAndroidPermissions(): Promise<PermissionResult> {
  try {
    const rn = require('react-native');
    if (rn?.Platform?.OS !== 'android') return { granted: [], denied: [], unavailable: true };
    const sdk = Number(rn.Platform.Version ?? 0);
    const perms = androidPermissionsFor(sdk);
    const res = await rn.PermissionsAndroid.requestMultiple(perms);
    const granted: string[] = [];
    const denied: string[] = [];
    for (const perm of perms) (res?.[perm] === 'granted' ? granted : denied).push(perm);
    return { granted, denied, unavailable: false };
  } catch {
    return { granted: [], denied: [], unavailable: true };
  }
}
...
export class BleplxRadio implements RadioAdapter {
  ...
  /** Set when start() refused to scan, so the UI can explain itself. */
  ...
  async start(events: RadioEvents): Promise<void> {
    ...
    this.blockedBy = null;
    if (!this.canScan) {
      this.blockedBy = 'react-native-ble-plx unavailable (needs a dev build, not Expo Go)';
      events.onError(new Error(this.blockedBy));
      return;
    }
    const perms = await ensureAndroidPermissions();
    if (perms.denied.length > 0) {
      this.blockedBy = 'Bluetooth permission denied: ' + perms.denied.map((p) => p.split('.').pop()).join(', ');
      events.onError(new Error(this.blockedBy));
      return; // scanning now would only throw SecurityException
    }
    try {
      const state = await this.mgr.state();
      if (state !== 'PoweredOn') {
        this.blockedBy = `Bluetooth is ${state.toLowerCase()} — turn it on`;
        events.onError(new Error(this.blockedBy));
        return;
      }
    } catch (err) {
      this.blockedBy = String((err as Error)?.message ?? err);
      events.onError(err);
      return;
    }
    this.startScan();
...
export class NativePeripheralRadio implements RadioAdapter {
  ...
  /** Set when start() refused to serve, so the UI can explain itself. */
  blockedBy: string | null = null;
