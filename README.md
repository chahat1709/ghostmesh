...
## If a build shows "0 nearby"

Every one of these has to hold, and each is a real failure mode:

1. **A dev/release build, not Expo Go** — Expo Go cannot contain the Bluetooth
   native module, so `BleplxRadio.canScan` is false and the radio never starts.
2. **Bluetooth actually on** — `BleManager.state()` must be `PoweredOn`.
3. **Runtime permissions granted.** Declaring `BLUETOOTH_SCAN` /
   `BLUETOOTH_CONNECT` / `BLUETOOTH_ADVERTISE` in the manifest is *not* enough:
   from API 31 they are runtime permissions and ble-plx does not ask for them.
   `ensureAndroidPermissions()` requests them on start (pre-31: fine location).
...
retry"), and tapping re-requests and restarts the radio. Previously a refusal
only reached `console.warn` in dev, so a phone with no permissions looked
exactly like a protocol that doesn't work.
